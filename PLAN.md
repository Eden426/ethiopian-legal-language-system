...
| A1-01 | P0 | Confirm canonical corpus schema, review states, and legacy mapping | student team | `src/corpus/`, `src/rag/legacy_jsonl.py`, `tests/corpus/`, `docs/DATASET_CARD.md`, `README.md` | A0-03 | in review | 25 focused schema/migration tests, 36 isolated branch tests, and 57 combined tests pass; strict review states, stable IDs, exact Unicode preservation, provenance, duplicate rejection, and unresolved legacy mapping verified; Ruff and compile pass |
| A1-02 | P0 | Define private local artifact storage, background tasks, retention, and job states | student team | `src/common/`, `src/api/`, `configs/`, `tests/` | A1-01 | in review | local SQLite job lifecycle, configurable private artifact layout and retention, content-safe logging, legacy-schema migration, artifact ignore, and 57 combined tests pass; merge remains gated by A1-01 review |
| A1-03 | P0 | Implement safe paired-PDF upload and queued job status API | student team | `src/api/`, `src/common/`, `src/corpus/ingestion/`, `frontend/`, `tests/`, `README.md` | A1-02 | in review | 19 focused upload/storage tests and 57 combined tests pass for proclamation/book metadata, private paired upload, canonical checksum-derived document IDs, legacy SQLite migration, malformed/type/size/page/mismatch rejection, paths, and state; Ruff, compile, and frontend build pass; queue transition awaits A1-04 |
