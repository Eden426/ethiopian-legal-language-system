...
| A1-02 | P0 | Define private local artifact storage, background tasks, retention, and job states | student team | `src/common/`, `src/api/`, `configs/`, `tests/` | A1-01 | in review | local SQLite job lifecycle, configurable private artifact layout and retention, content-safe logging, legacy-schema migration, and artifact ignore checks pass; merge remains gated by A1-01 review |
| A1-03 | P0 | Implement safe paired-PDF upload and queued job status API | student team | `src/api/`, `src/common/`, `src/corpus/ingestion/`, `frontend/`, `tests/`, `README.md` | A1-02 | in review | 19 focused upload/storage tests and 34 full tests pass for proclamation/book metadata, private paired upload, legacy SQLite migration, malformed/type/size/page/mismatch rejection, paths, and state; Ruff, compile, and frontend build pass; queue transition awaits the A1-04 processor |
