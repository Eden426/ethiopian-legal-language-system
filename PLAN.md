...
| A1-01 | P0 | Confirm canonical corpus schema, review states, and legacy mapping | student team | `src/corpus/`, `src/rag/legacy_jsonl.py`, `tests/corpus/`, `docs/DATASET_CARD.md`, `README.md` | A0-03 | done | student review confirmed; 25 focused schema/migration tests, 36 isolated branch tests, and 57 combined tests pass; strict review states, stable IDs, exact Unicode preservation, provenance, duplicate rejection, and unresolved legacy mapping verified; Ruff and compile pass |
| A1-02 | P0 | Define private local artifact storage, background tasks, retention, and job states | student team | `src/common/`, `src/api/`, `configs/`, `tests/` | A1-01 | done | student review confirmed; local SQLite job lifecycle, configurable private artifact layout and retention, content-safe logging, legacy-schema migration, artifact ignore, and 57 combined tests pass |
| A1-03 | P0 | Implement safe paired-PDF upload and queued job status API | student team | `src/api/`, `src/common/`, `src/corpus/ingestion/`, `frontend/`, `tests/`, `README.md` | A1-02 | in review | paired upload now queues background rendering; GET status returns content-safe stage, bounded progress, safe error code, file checksums, and ordered page metadata; 24 focused and 62 full tests pass; Ruff, compile, and frontend build pass |
| A1-04 | P0 | Render PDFs into ordered page images and record artifact checksums | student team | `src/corpus/ingestion/`, `src/common/`, `src/api/`, `frontend/`, `tests/`, `README.md` | A1-03 | in review | one-page-at-a-time Poppler rendering preserves originals; synthetic 2-page/1-page integration verifies role/order/count/dimensions/SHA-256; failure test verifies partial cleanup and safe status; 24 focused and 62 full tests pass |
| A1-05 | P0 | Add replaceable Amharic and English OCR providers | student team | `src/corpus/ocr/`, `src/common/`, `src/api/`, `configs/`, `tests/`, `README.md` | A1-04 | in review | replaceable offline Tesseract provider maps explicit `amh_Ethi`/`eng_Latn` settings; exact UTF-8 artifacts retain page/output checksums, confidence, engine/version, and safe failure state; 31 focused and 69 full tests, Ruff, compile, frontend build, and local synthetic bilingual smoke pass; student-reviewed public legal sample remains required |
